...
class AcknowledgePurchaseOrderView(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        instance = self.get_object()
        instance.acknowledgment_date = request.data.get('acknowledgment_date') or timezone.now()
        instance.save()
        ...
        ).aggregate(avg_response_time=Avg('acknowledgment_date') - F('issue_date'))

        # Update vendor's average_response_time
        instance.vendor.average_response_time = response_times['avg_response_time'] or 0
